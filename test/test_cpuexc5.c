...
/* 
 *		CPU exception test (5)
 *
 * [test purpose]
 *
 *  test the states when cpu exception happens in task context, 
 *  interrupts are unlocked and cpu is locked. test with ipm = 
 *  TIPM_ENAALL, task dispatch disabled, task exception enabled.
 *
 * [test items]
 *
 * 	when cpu exception happens in task context, interrupts are
 * 	unlocked and cpu is locked.
 *
