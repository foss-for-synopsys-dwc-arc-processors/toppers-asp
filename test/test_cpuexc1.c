...
/* 
 *		CPU exception test (1)
 *
 * [test purpose]
 *
 *  test the states when cpu exception happens in non-task context and
 *  interrupts are locked. test with ipm = TIPM_ENAALL, task dispatch
 *  disabled, task exception enabled.
 *
 * [test items]
 *
 * 	when cpu exception happens in non-task context and
