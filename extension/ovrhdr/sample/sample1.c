 ...
 *  task is running.
 *
 *  In the cyclic handler the ready queues of 3 task priorities (HIGH_PRIORITY,
 *  MID_PRIORITY, and LOW_PRIORITY) is rotated. The cyclic handler is stooped
 ...
 *  'V' : get the system time for performance evaluation by get_utm() twice.
 *  'o' : the target task overrun handler start operate
 *  'O' : the target task overrun handler stop operate
