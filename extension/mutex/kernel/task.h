...
#define TS_WAIT_MTX		(0x0aU << 3)	/* wait to get memory mutex */
 ...
 *
 */
#define TS_WAIT_MASK	(0x0fU << 3)	/* the macro to get the waiting reason */
 ...
 *  registered¡¥This time event block is a local variable in service calls
...
#else /* UINT8_MAX */
	BIT_FIELD_UINT	tstat : 8;		/* task status(bit field¡Ë*/
#endif /* UINT8_MAX */
 ...
 *
 *  When dispatcher is disabled or suspended, it's not limited that
 ...
 *  shows that a dispatch or some task's exception handling is needed
 */
 ...
 *  operations, or the priorities are few, maybe it's not very efficient.
 *
 *  In default, there are 16 priorities,  ready_primap's type is uint16_t
 */
