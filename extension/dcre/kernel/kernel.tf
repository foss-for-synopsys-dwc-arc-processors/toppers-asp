...
$ the number of statically created task
...
$ generate the initialization block table of dynamically created tasks
$IF num_atskid > 0$
...
$ the number of statically created semaphores
...
$ generate the initialization block table of dynamically created semaphores
$IF num_asemid > 0$
...
$ the number of statically created eventflags
...
$  generate the initialization block table of dynamically created eventflags
$IF num_aflgid > 0$
...
$ the number of statically created dataqueues
...
$ generate the initialization block table of dynamically created dataqueues
$IF num_adtqid > 0$
...
$ the number of statically created priority dataqueues
...
$ generate the initialization block table of dynamically created priority dataqueues
$IF num_apdqid > 0$
...
$ the number of statically created mailboxs
...
$ generate the initialization block table of dynamically created mailboxs
$IF num_ambxid > 0$
...
$ the number of statically created fixed-sized memorypools
...
$ generate the initialization block table of dynamically created fixed-sized memorypools
$IF num_ampfid > 0$
...
$ the number of statically created cyclic handlers
...
$ generate the initialization block table of dynamically created cyclic handlers
$IF num_acycid > 0$
...
$ the number of statically created alarm handlers
...
$ generate the initialization block table of dynamically created alarm handlers
$IF num_aalmid > 0$
...
$ error check of interrupt service routine (ISR)
...
$ generate ISR realated data structure
$intno_isr_list = {}$
$FOREACH intno INTNO_ATTISR_VALID$
	$inhno = INHNO[intno]$
...
$ generate the interrupt handler to call the ISRs
...
$ generated the initialization block table of dynamically created ISRs
$IF num_aisrid > 0$
...
$ generate the ISR control block table for all ISRs
...
$		// allocate the kernel memory area by configurator
