 ...
 *
 *  This file includes service call declarations, data types, constants and macros of
 *  TOPPERS/ASP kernel.
 *
...
/*
 *  definitions of packet format
 */
typedef struct t_ctsk {
	ATR			tskatr;		/* task attribute */
	intptr_t	exinf;		/* task extend information */
	TASK		task;		/* task address*/
	PRI			itskpri;	/* task init priority */
	SIZE		stksz;		/* task stack size */
	STK_T 		*stk;		/* tast stack */
} T_CTSK;
...
typedef struct t_dtex {
	...
	TEXRTN	texrtn;		/*task exception address */
} T_DTEX;
...
typedef struct t_csem {
	...
	uint_t	isemcnt;	/*  semaphore resources initial number */
	uint_t	maxsem;		/* semaphore resources maximum number */
} T_CSEM;

typedef struct t_rsem {
	ID		wtskid;		/* ID of the task at the head of the semaphore's wait queue */
...
typedef struct t_rflg {
	ID		wtskid;		/*  ID of the task at the head of the eventflag's wait queue */
	FLGPTN	flgptn;		/* current eventflag bit pattern */
} T_RFLG;

typedef struct t_cdtq {
	ATR		dtqatr;		/* data queue attribute */
	uint_t	dtqcnt;		/* the number of data can be stored in data queue management area */
	void 	*dtqmb;		/* the start address of the queue data management area */
} T_CDTQ;

typedef struct t_rdtq {
	ID		stskid;		/* ID of the task at the head of the data queue's send-wait queue */
	ID		rtskid;		/* ID of the task at the head of the data queue's receive-wait queue*/
	uint_t	sdtqcnt;	/* the number of data elements in the data queue */
} T_RDTQ;

typedef struct t_cpdq {
	ATR		pdqatr;		/* priority data queue attribute */
	uint_t	pdqcnt;		/* the number of data can be stored in a priority queue data management area */
	PRI		maxdpri;		/* the maximum value of priority data can be sent to the priority queue data */
	void 	*pdqmb;		/* the start address of the data management area a priority queue */
} T_CPDQ;

typedef struct t_rpdq {
	ID		stskid;		/* ID of the task at the head of the priority data queue's send-wait queue */
...
typedef struct t_cmbx {
	...
	PRI		maxmpri;	   /* the maximum value of priority messages can be sent priority mail box*/
	void 	*mprihd;	/* the start address of the header area of a different message queue priority */
} T_CMBX;
...
typedef struct t_cmpf {
	...
	uint_t	blkcnt;		/* number of fixed-sized memory pool that can be acquired */
	uint_t	blksz;		/* size of fixed-sized memory pool */
	MPF_T 	*mpf;		/* start address of fixed-sized memory pool */
	void 	*mpfmb;		/* start address of the fixed-sized memory pool management area */
} T_CMPF;
...
typedef struct t_ccyc {
	...
	intptr_t	exinf;		/* cyclic handler extended information */
	CYCHDR		cychdr;		/* cyclic handler start address*/
	RELTIM		cyctim;		/* cyclic handler initial period */
	RELTIM		cycphs;		/* cyclic handler initial phase  */
} T_CCYC;

typedef struct t_rcyc {
	STAT	cycstat;	/* cyclic handler state */
	RELTIM	lefttim;	/* cyclic handler remaining time */
} T_RCYC;

typedef struct t_calm {
	ATR			almatr;		/* alarm handler attribute */
	intptr_t	exinf;		/* alarm handler extended information*/
	ALMHDR		almhdr;		/* alarm handler start address*/
} T_CALM;
...
typedef struct t_cisr {
	...
	intptr_t	exinf;		/* interrupt service extended information */
...
#define TTW_SEM			UINT_C(0x0004)	/* waiting state for a semaphore resource *//* waiting state for a semaphore resource */
...
#define TOPPERS_SUPPORT_DYNAMIC_CRE		/* dynamic create extension */
