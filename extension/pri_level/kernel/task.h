 ...
 *  registedè™®è™ÀThis time event block is a local variable in service calls
 ...
 *
 *  when task is released from waiting stateè™®è¢the waiting result is stored
 ...
 *  		p_tinibè™®è¢tstatè™®è¢actque
 *  # valid except dormant state(dormant state is default status)
 *  		priorityè™®è¢wupqueè™®è¢enatexè™®è¢texptn
 ...
 *
 *  the number of priority determine the level of priority readymap.\
 *  if there is not enough, ready_primap2 is not supported.
 */
#define TBIT_PRIMAP		16				/* the number of stages per one bitmap */

#if TNUM_TPRI <= TBIT_PRIMAP
#define PRIMAP_LEVEL_1					/* Bitmap of 1 level */
#elif TNUM_TPRI <= TBIT_PRIMAP * TBIT_PRIMAP
#define PRIMAP_LEVEL_2					/* Bitmap of 2 levels */
...
extern uint16_t	ready_primap;					/* bitmap for ready queue */
#else /* PRIMAP_LEVEL_1 */
extern uint16_t	ready_primap1;					/* The 1st step of priority bitmap */
extern uint16_t	ready_primap2[TNUM_PRIMAP2];	/* The 2st step of priority bitmap */
